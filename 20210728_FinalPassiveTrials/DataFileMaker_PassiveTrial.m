        ...
        for gammaIndex = 1:length(gammaArray)
            ...
            for N0Index = 1:length(N0Array)
                ...
                for epsIndex = 1:length(epsArray)
                    ...
                    for j = 1:Rounds 
                        if (j == 1)
                            imagesc(0:Nmax, 0:Nmax, ProbMat)
                            colormap(cmap)
                            colorbar
                            axis square
                            xlabel('Initial Photons')
                            ylabel('Current Photons')
                            %title(strcat('Bayesian Posterior Estimate N=', int2str(N0), ', Round=', int2str(j)))
                            fname = strcat(figureFolder, 'PassiveTrialProb', sprintf('R%dN%deta%dnu%dgamma%d',j,N0,etaIndex,nuIndex,gammaIndex));
