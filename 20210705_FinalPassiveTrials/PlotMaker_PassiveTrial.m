        ...
        for gammaIndex = 1:length(gammaArray)
            ...
            for N0Index = 1:length(N0Array)
                ...
                for epsIndex = 1:length(epsArray)
                    ...
                    plot(1:Rounds, TrialStats(:,6),'-','Linewidth',2,'Color',[1 0.1 0.1])
                    plot(1:Rounds, TrialStats(:,7),'-','Linewidth',2,'Color',[0.6 0.1 1])
                    leg = {
                        'Init Photons'
                        'Init Photons Mean Estimate'
                        'Init Photons MLE'};
                    legend(leg,'FontSize',legfont,'Location','NorthEast');
                    xlabel('Rounds')
                    ylabel('Photons')
                    %title(strcat('Passive Trial Init N=', int2str(N0)))
                    fname = strcat(figureFolder, 'PassiveTrialInit', sprintf('N%deta%dnu%dgamma%d',N0,etaIndex,nuIndex,gammaIndex));
                    ...
                    grid off
                    colormap parula
                    plot(1:Rounds, TrialStats(:,2),'-','Linewidth',3,'Color',[0 0 0])
                    plot(1:Rounds, TrialStats(:,3),'-','Linewidth',2,'Color',[1 0.1 0.1])
                    plot(1:Rounds, TrialStats(:,4),'-','Linewidth',2,'Color',[0.6 0.1 1])
                    leg = {
                        'Loop Photons'
                    ...
                    xlabel('Rounds')
                    ylabel('Photons')
                    %title(strcat('Passive Trial Loop N=', int2str(N0)))
                    fname = strcat(figureFolder, 'PassiveTrialLoop', sprintf('N%deta%dnu%dgamma%d',N0,etaIndex,nuIndex,gammaIndex));
                    ...
                    xlim([1 Rounds])
                    set(gca,'YDir','normal')
                    plot(1:Rounds, TrialStats(:,8),'-','Linewidth',2,'Color',[0 0 0])
                    plot(1:Rounds, TrialStats(:,9),'-','Linewidth',2,'Color',[0 0.5 1])
                    leg = {
                    ...
                    fname = strcat(figureFolder, 'PassiveTrialInformation', sprintf('N%deta%dnu%dgamma%d',N0,etaIndex,nuIndex,gammaIndex));
